...
// Generated 21-Jul-2021 13:13:01
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"followerstopperth4rl\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(followerstopperth4rl_M, (NULL));
        followerstopperth4rl_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(followerstopperth4rl_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(followerstopperth4rl_M)
                == (NULL)) || rtmGetStopRequested(followerstopperth4rl_M));
    ...
    void NodeInterface::terminate(void)
    {
      if (mBaseRateThread.get()) {
        ...
        followerstopperth4rl_terminate();
        mNode.reset();
      }
    }
    ...
    {
      mRunModel = (rtmGetErrorStatus(followerstopperth4rl_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
